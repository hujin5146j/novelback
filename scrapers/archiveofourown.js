...
async function scrapeNovel(novelUrl, limit = 25, onProgress = null) {
  try {
    ...
    const novelTitle = $("h2.title").text().trim() ||
                       $("h1").first().text().trim() ||
                       "Unknown Novel";

    const chapterLinks = [];
    $("ol.chapter.contents li a").each((_, el) => {
    ...
    throw new Error(`Failed to scrape Archive of Our Own: ${err.message}`);
  }
}
